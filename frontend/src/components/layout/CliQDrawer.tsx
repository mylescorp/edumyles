    ...
    <div className="relative shrink-0">
      <div
        className={cn(
          ...
          item.isChannel
            ? "bg-[#1a4a30] text-[#6B9E83]"
            : "bg-[#0F4C2A] text-[#E8A020]"
        )}
      >
        {item.initials}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between">
          ...
          {item.unread && item.unread > 0 ? (
            <span className="ml-1 shrink-0 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-[#E8A020] text-[9px] font-bold text-[#061A12] px-1">
              {item.unread}
            </span>
          ) : null}
        </div>
...
export function CliQDrawer({
  ...
  return (
    <div
      ...
      style={{
        background: "#071e10",
        borderRight: "1px solid rgba(232,160,32,0.1)",
      }}
    >
      {/* Header — user row */}
      <div
        className="flex items-center justify-between px-3 py-2.5 border-b"
        style={{ borderColor: "rgba(232,160,32,0.1)" }}
      >
        <div className="flex items-center gap-2 min-w-0">
          <div className="relative shrink-0">
            <div className="h-7 w-7 rounded-full bg-[#0F4C2A] flex items-center justify-center text-[10px] font-bold text-[#E8A020]">
              {userName
                .split(" ")
                .map((w) => w[0])
                .join("")
            ...
            <span className="absolute bottom-0 right-0 h-2 w-2 rounded-full bg-emerald-400 border border-[#071e10]" />
          </div>
        ...
        <div className="flex items-center gap-0.5 shrink-0">
          <button className="h-6 w-6 flex items-center justify-center rounded text-white/40 hover:text-[#E8A020] hover:bg-white/8 transition-colors">
            <Plus className="h-3.5 w-3.5" />
          </button>
          <button
            onClick={onClose}
            className="h-6 w-6 flex items-center justify-center rounded text-white/40 hover:text-[#E8A020] hover:bg-white/8 transition-colors"
          >
      ...
      <div
        ...
        style={{ borderColor: "rgba(232,160,32,0.08)" }}
      >
        <div className="relative">
          ...
          <input
            ...
            className="w-full h-7 pl-7 pr-2 text-[11px] rounded-md bg-white/6 border border-white/8 text-white/80 placeholder:text-white/30 outline-none focus:border-[rgba(232,160,32,0.3)] transition-colors"
          />
        </div>
      </div>
