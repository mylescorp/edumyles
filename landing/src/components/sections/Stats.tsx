...
export default function Stats() {
  const schools = [
    "Nairobi Green Academy", "St. Mary's Girls High", "Brookside Prep",
    "Upperhill School", "Strathmore School", "St. Francis Kisumu",
    "Hillcrest School", "Starehe Girls' Centre", "Alliance High School",
    "Kenya High School", "Aga Khan Academy", "Braeburn School",
  ];
  ...
  return (
    <section
      ...
      style={{ background: "#061A12", borderTop: "3px solid #E8A020", borderBottom: "1px solid rgba(232,160,32,0.15)" }}
    >
      {/* ── Header ── */}
      <div className="max-w-[1200px] mx-auto px-6 pt-14 pb-8 text-center">
        ...
        >
          ...
          <em className="italic" style={{ color: "#E8A020" }}>run on.</em>
        </h2>
        <p className="font-jakarta text-[14px]" style={{ color: "#6B9E83", maxWidth: 480, margin: "0 auto" }}>
          From Nairobi to Kampala to Lusaka — schools across East Africa choose EduMyles to manage their students, fees, and staff.
        </p>
      </div>
          ...
          {stats.map((s) => (
            ...
            >
              <div
                ...
                style={{ fontSize: "clamp(1.6rem,2.5vw,2.25rem)", color: "#E8A020", lineHeight: 1.1 }}
              >
                {s.value}
              </div>
              <div className="font-jakarta font-semibold text-[14px] text-white mb-0.5">{s.label}</div>
              <div className="font-jakarta text-[11px]" style={{ color: "#6B9E83" }}>{s.sub}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* ── Countries ── */}
      <div
        className="max-w-[1200px] mx-auto px-6 pb-12 flex flex-wrap justify-center items-center gap-3"
      >
        <span className="font-jakarta font-semibold text-[11px] uppercase tracking-[2px]" style={{ color: "#4a6b58" }}>
          Operating in:
        </span>
        {countries.map((c) => (
          <span
            ...
            style={{ background: "rgba(168,230,195,0.08)", border: "1px solid rgba(168,230,195,0.15)", color: "#A8E6C3" }}
          >
            {c.flag} {c.name}
          </span>
        ))}
