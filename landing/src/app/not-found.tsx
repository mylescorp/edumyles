...
export default function NotFound() {
  return (
    ...
    >
      ...
      {/* Giant 404 watermark */}
      <div
        className="absolute select-none pointer-events-none font-playfair font-bold"
        style={{ fontSize: "clamp(150px, 30vw, 280px)", color: "rgba(232,160,32,0.07)", lineHeight: 1, top: "50%", left: "50%", transform: "translate(-50%, -50%)", whiteSpace: "nowrap" }}
        aria-hidden="true"
      >
        404
      </div>
      ...
      <div className="relative z-10 max-w-[640px] w-full text-center">
        ...
        {/* Logo */}
        <Link href="/" className="inline-flex items-center gap-3 mb-10 no-underline" aria-label="EduMyles home">
          <div
            className="w-12 h-12 flex items-center justify-center rounded-[14px] font-playfair font-bold text-2xl"
            style={{ background: "linear-gradient(135deg,#0F4C2A,#1A7A4A)", border: "2px solid #E8A020", color: "#E8A020" }}
          >
            E
          </div>
          <span className="font-playfair font-bold text-[20px] text-white">EduMyles</span>
        </Link>

        <h1 className="font-playfair font-bold text-white mb-4" style={{ fontSize: "clamp(2rem,5vw,3rem)" }}>
          Page not found
        </h1>
        <p className="font-jakarta text-[16px] leading-[1.8] mb-10" style={{ color: "#90CAF9", maxWidth: 480, margin: "0 auto 2.5rem" }}>
          The page you&apos;re looking for has moved, been removed, or doesn&apos;t exist. Let&apos;s get you back on track.
        </p>

        {/* Quick links */}
        <p className="font-jakarta font-semibold text-[12px] uppercase tracking-[2px] mb-5" style={{ color: "#6B9E83" }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-10">
          {quickLinks.map(({ label, href, icon: Icon }) => (
            ...
            >
              <div className="w-8 h-8 rounded-[8px] flex items-center justify-center flex-shrink-0" style={{ background: "rgba(232,160,32,0.1)" }}>
