        ...
        try {
            ...
            if (args.orgId && (tenant.role === "master_admin" || tenant.role === "super_admin")) {
                ...
                const targetTenant = await ctx.db
                    .query("tenants")
                    .withIndex("by_tenantId", (q) => q.eq("tenantId", args.orgId!))
                    .first();
            ...
            if (args.sortBy) {
                ...
                filteredStudents.sort((a, b) => b.createdAt - a.createdAt);
            }
            ...
            if (args.cursor) {
                const cursorIndex = filteredStudents.findIndex(
                    student => student._id === args.cursor
                );
                if (cursorIndex !== -1) {
                ...
                paginatedStudents.map(async (student) => {
                    ...
                    if (student.guardianUserId) {
                        const guardian = await ctx.db
                            .query("users")
                            .withIndex("by_userId", (q) => q.eq("userId", student.guardianUserId!))
                            .first();
            ...
            return {
                ...
                nextCursor: paginatedStudents.length > 0 
                    ? paginatedStudents[paginatedStudents.length - 1]._id 
                    : null,
            };
...
export const listStudentsBySchool = query({
    ...
    handler: async (ctx, args) => {
        const result = await getStudentsBySchool(ctx, args);
